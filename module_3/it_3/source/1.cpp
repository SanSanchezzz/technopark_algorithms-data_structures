...
std::vector<size_t> shortestPath(const ListGraph& graph, size_t from);
...
int main() {
    ...
    std::cin >> from >> to;
    std::vector<size_t> result = shortestPath(graph, from);
    std::cout << result[to] << std::endl;

    return 0;
}

std::vector<size_t> shortestPath(const ListGraph& graph, size_t from) {
    std::vector<size_t> weight(graph.verticesCount(), UINT_MAX);
    ...
    while (!q.empty()) {
        ...
        for (Edge child : next_vertices) {
            if (weight[child.first] > weight[v] + child.second) {
                if (weight[child.first] != UINT_MAX) {
